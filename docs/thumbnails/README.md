...
카카오톡은 카드 이미지를 **2:1 로 센터 크롭**한다. 실제 공유 결과로 확인했다.
1.596:1 이미지를 넣었더니 상하 20% 가 잘려 태그라인과 날짜가 사라졌다.
카카오 공식 권장 규격도 800x400(2:1) 이다.

주 공유 채널이 카카오톡이므로 **2:1 을 기준으로 만든다.**
페이스북·X 의 `summary_large_image` 는 1.91:1 이라 좌우가 4.5% 잘리지만,
그 영역은 배경 여백이라 문구에 닿지 않는다.
